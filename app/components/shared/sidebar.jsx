    ...
    <aside className="w-48 fixed left-0 top-0 overflow-y-auto h-screen  bg-primary text-white p-4 gap-5 flex flex-col">
      ...
      <ul className="flex flex-col gap-2">
        {SIDEBAR_TABS.map((tab) => {
          ...
          const active = isParentActive(tab);

          return (
            <li key={id}>
              {/* Parent Tab */}
              <div
                className={`flex items-center justify-between py-2 pr-3 pl-4 h-10 w-40 rounded-full cursor-pointer
                  ${
                    active
              ...
              >
                ...
                {children && (open ? <VscChevronUp /> : <VscChevronDown />)}
              </div>

              {/* Sub Tabs */}
              {children && open && (
                <ul className="border-l border-white ml-7 mt-2">
                  {children.map((child) => {
                    const childActive = isTabActive(child.route);

                    return (
                      <li key={child.id}>
                        <Link
                          href={child.route}
                          className={`flex ml-3 items-center py-2 pr-3 pl-4 rounded-full text-[14px]
                            ${
                              childActive
                                ? "bg-secondary text-black"
                                : "hover:bg-secondary hover:text-black"
        ...
        <div className="w-40 h-53 bg-primary-light rounded-2xl p-4 flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="absolute top-0 right-0 ">
            ...
            >
              <path
                opacity="0.5"
                d="M21.3094 -10.6663L0.50628 39.063C-1.27103 43.3116 1.8498 48 6.45518 48C9.03852 48 11.8371 46.2222 12.9135 45.3334C19.9181 38.1029 28.2732 32.9637 34.0727 30.6717C37.1608 29.4512 40.1733 31.3847 41.3544 34.488L45.0931 44.3116C45.9382 46.5322 48.0673 48 50.4434 47.9999C54.4578 47.9998 57.2299 43.9881 55.7923 40.24C50.7881 27.1933 40.8305 1.25181 36.7098 -9.34652C35.5988 -12.204 32.9239 -13.9999 29.8581 -13.9998L26.476 -13.9996C23.8927 -13.9996 21.9552 -11.9994 21.3094 -10.6663Z"
                fill="#00413D"
              />
            </svg>
          </div>
          ...
          {/* Button */}
            ...
            <Button variant="secondary">Get Pro</Button>
        </div>
      </div>
    </aside>
  );
